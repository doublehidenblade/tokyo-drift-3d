...
// VM HYGIENE: unique user-data-dir prefix /tmp/wave-climb-sg-*, cleanup pkills scoped to it.
'use strict';
...
const VIEWS = [
  { name: 'signage', night: true, view: [1.98, 1.50, 7.6, -2, 3, 41] },
  { name: 'street2', night: true, street: true },
];
...
async function main() {
  ...
  try {
    chrome = spawn(CHROME, ['--headless=new', '--no-sandbox', '--disable-gpu', '--disable-dev-shm-usage',
      ...
      '--window-size=1600,900', '--hide-scrollbars', url], { stdio: 'ignore' });
    let wsUrl = null;
    for (let i = 0; i < 120; i++) {
      await sleep(500);
    ...
    if (!ready) throw new Error('ready timeout 600s');
